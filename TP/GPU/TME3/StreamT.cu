/**************************************************************
This code shows how to program asynchronous transfer and 
...
void withStream (int *aCPU, int *bCPU, int size, int NBS){
	...
	for (int i=0; i<size; i+= NBS*F){
	  for (int j=0; j <NBS; j++){
	 	testCUDA(cudaMemcpyAsync(a+i+j*F,aCPU+i+j*F,L,cudaMemcpyHostToDevice,stream[j]));
		testCUDA(cudaMemcpyAsync(b+i+j*F,bCPU+i+j*F,L,cudaMemcpyHostToDevice,stream[j]));
	  }

	  for (int j=0; j <NBS; j++){
		//if(j%2){	// Decomment to show the concurrent execution of 2 different kernels
			add_k<<<(F+1023)/1024,1024,0,stream[j]>>>(a+i+j*F,b+i+j*F,F);
		//}else{	// Decomment to show the concurrent execution of 2 different kernels
		//	sub_k<<<(F+1023)/1024,1024,0,stream[j]>>>(a+i+j*F,b+i+j*F,F); // also here 
		//}			// Decomment to show the concurrent execution of 2 different kernels
	  }
	  ...
	  for (int j=0; j <NBS; j++){
		testCUDA(cudaMemcpyAsync(cCPU+i+j*F,b+i+j*F,L,cudaMemcpyDeviceToHost,stream[j]));
	  }
	}
...
int main (void){
	...
	// Only one transfer
	withoutStream (a, b, size, -1);
	// Transfer 128 frames without streams: Processing them NBS by NBS frames
	withoutStream (a, b, size, NBS);
	// Transfer 128 frames using NBS streams: Processing them NBS by NBS frames
