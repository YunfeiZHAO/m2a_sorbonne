...
class DDPGMultiAgent(object):
    def __init__(self, env, opt):
        ...
        self.agents = []
        for i in range(opt.nbAgent):
            self.agents.append(DDPGAgent(env, opt))

    def learn(self):
        """
        Train each agent for a given batch
        """
    ...
    def store(self, ob, action, new_obs, reward, done, it):
        ...
        :param ob, new_obs: list of numpy array for all agent
        :param action: list of numpy array for all agent
        :param r: value
        """
        ...
        if not self.test:
            ...
            tr = (ob, action, reward, new_obs, done)
...
if __name__ == '__main__':
    ...
    config, outdir, logger = init('./configs/config_DDPG.yaml', "DDPG")
