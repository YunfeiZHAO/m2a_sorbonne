...
def L_relu(x):
    return K.relu(x, alpha=0., max_value=None, threshold=1)
...
model.add(Dense(1,name="couche_sortie",
                kernel_initializer='random_uniform',
                use_bias=True, bias_initializer=init_b_sortie, 
                activation='linear'))

Wini=model.get_weights()


model.compile(loss='mse', optimizer=Adam(beta_1=0.9,beta_2=0.975))
