...
def init_W1(shape, dtype=None):
    ...
    for i in range(0,N): W[i,i]= -2/dx
    for i in range(1,N): W[i-1,i]=1/dx
    return K.constant(W)
...
model.add(Dense(N, input_dim=1,name="lay_hid",kernel_initializer=init_W1,
                use_bias=False,activation='relu'))

model.add(Dense(1,name="lay_out",kernel_initializer=init_W2,
